...
SHADOW_MARKETS: list[str] = [
    ...
    "KXMLS",      # MLS moneyline (3-way Shin de-vig) + total goals (Poisson),
...
_bad_match_ids = {
    ...
    "KXMLBGAME-26JUL302140SFSD-SF|YES",              # Phantom moneyline edge from the same-first-word team misparse (SF vs SD both "San"): the Giants market was tagged with San Diego's 59.3% favorite prob (Giants were a +141 underdog, true fair ~40.7%), manufacturing a ~14% edge vs Kalshi's correct 42c. Root cause fixed in kalshi_ev_scanner.py _parse_moneyline_team (bidirectional city-in-name match), commit 663e6cd; this removes the open bug-artifact bet it had already logged. Won't re-flag — SF now prices at ~0.41, no edge.
    "KXMLBKS-26AUG042140SDAZ-AZERODRIGUEZ57-6|YES",  # Phantom strikeout edge from cross-game name collision (2026-08-04): Eduardo Rodriguez (AZ, SD@AZ) was absent from the Pinnacle index, so _find_player_in_title's surname fallback borrowed Grayson Rodriguez's (BAL) "over 5.5" line → fake fair 41.0% vs Kalshi's correct 25c (Eduardo's true 6+ ≈ 25%, his real line is over 3.5) → bogus +13.2% edge. Verified against the Odds API. Root cause fixed in kalshi_ev_scanner.py: game/team-match guard in scan_player_props + surname-ambiguity refuse in _find_player_in_title + STRIKEOUT_MAX_EDGE 8% cap. Removes the open artifact bet; won't re-flag.
}
...
def _get_performance(since: Optional[str] = None) -> dict:
    ...
    for b in clean_settled:
        ...
        # Expected win rate = average KALSHI-implied win probability (the side
        # price we PAID) over the same live bets that produce win_rate. This is
        # the break-even rate: Win Rate above it = beating the market price we
        # paid, which is exactly what drives flat units. (Kalshi-implied, not the
        # Pinnacle model fair — those differ by the edge we captured at entry.)
        _ki_v = b.get("kalshi_price")
        if _ki_v is not None:
            by_type[label]["kalshi_implied"].append(_ki_v)
        if kp is not None:
    ...
    for label, d in by_type.items():
        ...
        # Expected win rate — avg KALSHI-implied win prob (price paid = break-even)
        expected_wr_t   = round(sum(d["kalshi_implied"]) / len(d["kalshi_implied"]) * 100, 1) if d["kalshi_implied"] else None
...
// MLS team logo for a moneyline/total ticker (e.g. KXMLSGAME-26JUL25SJLAG-SJ
// => player-side team not applicable; use the two team codes from the segment).
// For team markets the market suffix IS a team code (SJ/LAG/TIE); TIE has no
// logo. For totals the ticker carries no team suffix, so fall back to the first
// code in the segment. ESPN soccer logos are by numeric team id.
const _MLS_TEAM_ID = {ATL:'18418',ATX:'20906',MTL:'9720',CLT:'21300',CHI:'182',COL:'184',CLB:'183',DCU:'193',CIN:'18267',DAL:'185',HOU:'6077',MIA:'20232',LAG:'187',LAFC:'18966',MIN:'17362',NSH:'18986',NE:'189',NYC:'17606',NYRB:'190',ORL:'12011',PHI:'10739',POR:'9723',RSL:'4771',SD:'22529',SJ:'191',SEA:'9726',SKC:'186',STL:'21812',TOR:'7318',VAN:'9727'};
const _MLS_CODES = new Set(Object.keys(_MLS_TEAM_ID));
function mlsLogo(ticker) {
  if (!ticker) return '';
  const m = ticker.match(/^KXMLS(?:GAME|TOTAL)-\d{2}[A-Z]{3}\d{2}([A-Z]+)(?:-([A-Z]+))?/);
  if (!m) return '';
  const seg = m[1], suffix = m[2] || '';
  // Team markets: suffix is the team code (skip TIE — no single team).
  if (suffix && suffix !== 'TIE' && _MLS_TEAM_ID[suffix]) {
    return `<img src="https://a.espncdn.com/i/teamlogos/soccer/500/${_MLS_TEAM_ID[suffix]}.png" onerror="this.style.display='none'" style="width:18px;height:18px;vertical-align:middle;margin-right:6px;object-fit:contain;">`;
  }
  // Totals / TIE: show the home (first) team logo from the segment.
  for (const n of [4, 3, 2]) {
    const a = seg.slice(0, n), h = seg.slice(n);
    if (_MLS_CODES.has(a) && _MLS_CODES.has(h)) {
      return `<img src="https://a.espncdn.com/i/teamlogos/soccer/500/${_MLS_TEAM_ID[a]}.png" onerror="this.style.display='none'" style="width:18px;height:18px;vertical-align:middle;margin-right:6px;object-fit:contain;">`;
    }
  }
  return '';
}
...
function renderTodayEdges() {
  ...
  const rows = [...todayEdgesList].reverse().map(b => {
    ...
    const liveCounter = b.live_stat
      ? `<span style="font-weight:700;font-size:13px;color:var(--text);white-space:nowrap;" title="Live in-game — box stat for props (MLB Stats / ESPN), scoreline for MLS game lines (ESPN). Updates ~every minute.">${b.live_stat}</span>`
      : `<span style="color:var(--muted);">—</span>`;
...
function renderPerformance(d) {
  ...
  const renderPerfRow = (b) => {
    ...
    return `<tr style="${b.correlated || b.clv_source === 'corrupted_utc' ? 'opacity:0.55;' : isShadow ? 'opacity:0.75;border-left:2px solid #58a6ff22;' : ''}">
      <td>${ts}</td>
      <td>${gameTimeCell}</td>
      <td>${sportLogo(b.ticker)}${b.matchup}${corrBadge}${corruptBadge}${shadowBadge}</td>
...
async function fetchPaper() {
  try {
    ...
    // Portfolio ROI chart plots cumulative Kelly-sized $ P&L (user call 2026-07-31).
